...
public class MainActivity extends Activity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        StarThread twinkleThread = new StarThread(myAV);
        twinkleThread.start();

        StarAutoAddRemoveThread arThread = new StarAutoAddRemoveThread(myAV);
        arThread.start();
    }//onClick

    @Override
    public void onClick(View v) {
            Log.i("msg", "invalidate");
            myAV.postInvalidateDelayed(4000);

        /*final int timeLeft = 1000;
        CountDownTimer myTimer = new CountDownTimer(timeLeft, 10) {
            @Override
